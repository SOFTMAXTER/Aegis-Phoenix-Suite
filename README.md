# Aegis Phoenix Suite v4.9.0 by SOFTMAXTER
...
* **Centro de Diagnóstico de Red (GUI)**: Nueva interfaz gráfica completa para diagnósticos de red. Incluye pruebas de Ping, DNS, Tracert, renovación de IP y reset de Winsock/TCP-IP con consola de salida integrada en tiempo real.
* **Gestor de Claves Wi-Fi (GUI)**: Nueva herramienta visual para ver perfiles Wi-Fi guardados, desencriptar claves, exportar/importar backups (XML) y eliminar redes antiguas.
* **Limpieza de Navegadores**: Nuevo módulo para la limpieza profunda de cachés (incluyendo GPU Cache) de Chrome, Edge, Firefox, Brave y Opera, preservando cookies e historial.
* **Optimización de Unidades Interactiva**: El módulo de desfragmentación ahora detecta inteligentemente SSD vs HDD, verifica el estado de TRIM en el sistema y permite elegir unidades específicas o modos de solo análisis.
...
* **Gestor de Ajustes Visual y Dinámico**: GUI optimizada con buscador y filtrado por categorías para aplicar tweaks de rendimiento, seguridad y privacidad.
...
* **Limpieza Profunda e Interactiva**: Panel gráfico para seleccionar qué limpiar (Temporales, Cachés, Papelera, Sistema, **Navegadores**) con cálculo de espacio en tiempo real.
...
* **Herramientas de Diagnóstico y Respaldo**: Análisis inteligente de logs de eventos (con reportes HTML y soluciones), respaldo de datos de usuario (Robocopy) con validación anti-bucle y modos de espejo/mover.
* **Reubicación de Carpetas de Usuario**: Asistente para mover carpetas clave (Escritorio, Documentos) a otra ubicación de forma segura con actualización de registro.
...
Al iniciar, se presentan las categorías principales de la suite con información detallada del sistema en el encabezado.
...
* **2. Optimizar Servicios de Terceros (GUI)**: Panel visual que detecta servicios no nativos. Crea backups automáticos en JSON y permite alternar su estado.
* **3. Módulo de Limpieza Profunda (GUI)**: Interfaz gráfica que permite "Escanear" el espacio ocupado por temporales, cachés y `Windows.old`. Incluye ejecución robusta de `CleanMgr`.
...
* **5. Gestionar Programas de Inicio (GUI)**: Panel interactivo para habilitar/deshabilitar programas de arranque (Registro, Carpetas, Tareas).
...
* **3. Optimizar Unidades (Interactivo)**: Menú inteligente que detecta SSD (TRIM) y HDD (Desfragmentación). Permite analizar fragmentación sin ejecutar cambios y muestra el estado de TRIM en el SO.
...
* **6. Diagnostico y Reparacion de Red (GUI)**: **NUEVO**. Panel gráfico con herramientas de lectura (Ping, IPConfig, DNS) y reparación (Flush DNS, Renew IP, Reset Winsock/TCP-IP).
* **7. Reconstruir Índice de Búsqueda**: Purgado y regeneración de la base de datos de Windows Search.
* **8. Limpieza Profunda de Cache de Navegadores**: **NUEVO**. Elimina caches de Chrome, Edge, Firefox, Brave y Opera, cerrando los procesos automáticamente.
...
* **Gestionar Tareas Programadas de Terceros (GUI)**: Interfaz para habilitar/deshabilitar tareas programadas no nativas.
* **Reubicar Carpetas de Usuario**: Asistente seguro para mover carpetas personales y actualizar el registro.
* **Gestor de Claves Wi-Fi (GUI)**: **NUEVO**. Visualiza perfiles guardados, muestra contraseñas desencriptadas, permite exportar (Backup XML) y restaurar perfiles.
...
* **Base de Conocimientos**: **NUEVO**. Soluciones integradas para errores comunes (ID 153, 41, etc.).
* **Monitoreo en Tiempo Real**: **NUEVO**. Observa eventos críticos y errores a medida que ocurren en el sistema.

#### → Herramienta de Respaldo de Datos de Usuario

* Interfaz para `Robocopy` con modos Simple, Espejo y **Mover**.
